...
exports.api = exports.syncData = exports.cleanupOldData = exports.generateRecommendations = exports.analyzeUserInteractions = exports.processScholarships = exports.sendEmail = exports.sendNotification = void 0;
...
exports.processScholarships = functions.https.onCall(async (data, context) => {
    ...
    try {
        switch (action) {
            case 'create':
                const newScholarship = await admin.firestore().collection('scholarships').add(Object.assign(Object.assign({}, scholarshipData), { createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
                return { success: true, id: newScholarship.id };
            case 'update':
                await admin.firestore().collection('scholarships').doc(scholarshipId).update(Object.assign(Object.assign({}, scholarshipData), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
                return { success: true };
            case 'delete':
...
exports.analyzeUserInteractions = functions.https.onCall(async (data, context) => {
    // Vérifier l'authentification et les permissions admin
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Utilisateur non authentifié');
    }
    ...
    try {
        ...
        if (userData.interests.length > 0) {
            const scholarshipsSnapshot = await admin.firestore()
                ...
                .limit(5)
                .get();
            recommendations.scholarships = scholarshipsSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        }
